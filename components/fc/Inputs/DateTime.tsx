...
const DateTimeInput = React.memo((props: DateTimeInputProps) => {
  const normalizedProps: DateTimeInputProps = {
    ...props,
    inputClassName: 'datetime',
    showReadable: true,
  };

  const [type, setStep] = useState(normalizedProps.type);
  const [showReadable, setDecimals] = useState(normalizedProps.showReadable);
  ...
  return <Input
    ...
    renderValueElement={(input: any): React.JSX.Element => {
      ...
      return <div className='flex'>
        ...
        {showReadable ? <div className="text-xs">{renderReadableInfo(value)}</div> : null}
      </div>;
    }}
    ...
    renderInputElement={(input: any): React.JSX.Element => {
      ...
      switch (type) {
        case 'datetime':
          ...
          defaultPlaceholder = input.translate('Year-Month-Day Hour:Min:Sec','Hubleto\\Erp\\Loader','Components\\Inputs\\DateTime');
        break;
        case 'date':
          icon = 'fas fa-calendar';
          value = dateToEUFormat(value);
          options = {...options, showMonths: 2, weekNumbers: true, dateFormat: 'd.m.Y'};
          defaultPlaceholder = input.translate('Year-Month-Day','Hubleto\\Erp\\Loader','Components\\Inputs\\DateTime');
        break;
        case 'time':
          icon = 'fas fa-clock';
          options = {
      ...
      const readableInfo = (showReadable ? renderReadableInfo(value) : null);
