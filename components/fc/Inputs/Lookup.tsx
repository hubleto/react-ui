...
const LookupInput = React.memo((props: LookupInputProps) => {
  ...
  const [urlAdd, setUrlAdd] = useState(normalizedProps.urlAdd);
  const [uiStyle, setUiStyle] = useState(normalizedProps.uiStyle);
  const [model, setModel] = useState(normalizedProps.model);
  const [endpoint, setEndpoint] = useState(normalizedProps.endpoint);
  const [customEndpointParams, setCustomEndpointParams] = useState(normalizedProps.customEndpointParams);

  const getEndpointParams = (input: any): object => {
  ...
  const loadData = (input: any, searchValue: string|null = null) => {
    request.post(
      endpoint,
      {...getEndpointParams(input), search: searchValue},
      {},
      (data: any) => {
      ...
      if (uiStyle == 'select') {
        ...
        return <>
          <AsyncSelect
            ...
            loadOptions={(searchValue: string, callback: any) => loadData(searchValue, callback)}
